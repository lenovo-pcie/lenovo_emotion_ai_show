...
class EmotionAnalyzerLenovoVllmQwen32b(EmotionAnalyzerBase):
    def __init__(self, api_key, camera_backend=0, camera_index=0):
        super().__init__(api_key, camera_backend, camera_index, "Lenovo-Vllm-Qwen32b")
        # 远程服务配置
    ...
    def analyze_screen_content(self, screen_image):
        ...
        try:
            ...
            print(f"🖥️ 正在调用远程服务分析屏幕内容...")
            
            response = requests.post(f"{self.base_url}/generate", headers=headers, json=request_data, timeout=30)
            end_time = time.time()
            api_time = end_time - start_time
            
            if response.status_code == 200:
                result = response.json()
                # vLLM /generate 端点通常返回 'text' 字段
                if 'text' in result and len(result['text']) > 0:
                    response_text = result['text'][0]  # 取第一个生成的文本
    ...
    def analyze_emotion(self, images):
        ...
        try:
            print(f"🌐 正在调用远程服务分析 {len(images)} 张图片...")
            ...
            for i, image in enumerate(images):
                ...
                if response.status_code == 200:
                    ...
                    if 'text' in result and len(result['text']) > 0:
                        ...
                        print(f"❌ 图片 {i+1}/{len(images)} 未知响应格式: {result}")
